        ...
        <table class="table table-items">
            <thead>
                <tr>
                    ...
                    @foreach($invoice->currencies as $code => $symbol)
                    <th scope="col" class="text-right border-0">{{ __('invoices::invoice.price') . " " . __($code) }}</th>
                    @endforeach
                    ...
                    @endif
                    @foreach ($invoice->currencies as $code => $symbol)
                        <th scope="col" class="text-right border-0 pr-0">{{ __('invoices::invoice.sub_total') . ' ' . __($code) }}</th>
                    @endforeach
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    @foreach($invoice->currencies as $code => $symbol)
                        <td class="text-right">
                            @if($code == $item->currency_code)
                                {{ $item->price_per_unit . ' ' . $item->currency_symbol}} 
                            @endif
                        </td>
                    @endforeach
                    @if($invoice->hasItemDiscount)
                    ...
                    @foreach($invoice->currencies as $code => $symbol)
                        <td class="text-right pr-0">
                            @if($code == $item->currency_code)
                                {{ $item->sub_total_price . ' ' . $item->currency_symbol }}
                            @endif
                        </td>
                    @endforeach
                </tr>
                    ...
                    <tr>
                        ...
                        @foreach($invoice->currencies as $code => $symbol)
                            <td class="text-right pr-0 total-amount">
                                {{ ($invoice->total_amount[$code]) . ' ' . $symbol }}
                            </td>
                        @endforeach
                    </tr>
            </tbody>
