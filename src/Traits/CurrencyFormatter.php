...
trait CurrencyFormatter
{
     ...
     * @return string
     */
    public function getAmountInWords(float|array $amount, ?string $locale = null)
    {

        $amountInWord = [];
        if(is_array($amount)){
            foreach($amount as $currency_code => $total){
                $amountFormat = number_format($total, $this->currency_decimals, '.', '');
                $formatter = new NumberFormatter($locale ?? App::getLocale(), NumberFormatter::SPELLOUT);

                $value = explode('.', $amountFormat);
