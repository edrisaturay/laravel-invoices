...
namespace EdrisaTuray\Invoices\Traits;

use Exception;
use Illuminate\Support\Str;
use EdrisaTuray\Invoices\Contracts\PartyContract;
use EdrisaTuray\Invoices\Services\PricingService;
use Symfony\Component\HttpFoundation\File\File;

/**
 * Trait InvoiceHelpers.
 */
trait InvoiceMultipleHelpers
{
    /**
     * @return $this
     */
    ...
    public function calculate()
    {
        ...
        $this->currencies->each(
            function ($symbol, $code) use (&$total_amount, &$total_discount, &$total_taxes) {
                $total_amount[$code] = 0;
                $total_discount[$code] = 0;
                $total_taxes[$code] = 0;
            }
        );
        ...
        $this->items->each(
            function ($item) use (&$total_amount, &$total_discount, &$total_taxes) {
                ...
                if ($item->hasDiscount()) {
                    $total_discount[$item->currency_code] += $item->discount;
                    $this->hasItemDiscount = true;
                }

                if ($item->hasTax()) {
                    $total_taxes[$item->currency_code] += $item->tax;
                    $this->hasItemTax = true;
                }

                // Totals
                $total_amount[$item->currency_code] += $item->sub_total_price;
            }
        );
